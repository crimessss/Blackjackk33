...
const { handleHelpCommand } = require('./utils/helpCommands');
...
bot.on("chatCreate", async (user, message) => {
  try {
    ...
    if (message.toLowerCase() === '!help' || message.toLowerCase() === '!commands') {
      await handleHelpCommand(bot, user);
      return;
    }
...
bot.on("playerJoin", async (user) => {
  const welcomeMessage = 
    ...
    "Tip gold to get chips (1 Gold = 1 Chip)\n" +
    "Type !help to see all commands";
    
  bot.whisper.send(user.id, welcomeMessage);
});
